...
class(NTL.phys.data.PeterPaul$sampledate) # when uploading datasets from CSV, it doesn't store the data format. Therefore you often have to reset the variable to a date format  
...
summary(NTL.nutrient.data.PeterPaul$comments) # it includes comments that were in the original dataset
...
write.csv(NTL.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv") # do not want a file with the first collumn being row numbers; it will overide any file there with the same name 
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) # gather is a function that incleases the observations by combining the selected variables - tn_ug:po4 - into 2 collunms. One collumn is the original cariable name and the second the value
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) # removing the observations where the observation is NA
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) # Spreading a data set is function that splits the data in 2 collumns into several collumns 

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.skinny, sampledate, c("Y", "m", "d")) # separate is used to split the data in a collumn into different collumns. In this case separating the date variable into a Y, m and d collumn. It doesnt retain the initial date variable. New varibles will be stored asa character.
...
* `inner_join`: return rows in x where there are matching values in y, and all columns in x and y (mutating join). You have to specify which data set is first. Order is important.
...
* `full_join`: return all rows and all columns from x and y. Returns NA for missing values (mutating join). Most inclusive join. Doesn't leave anything out. 
...
NTL.phys.nutrient.data.PeterPaul <- select(NTL.phys.nutrient.data.PeterPaul, lakename, year4, month, sampledate:po4) # changing the order of columns
...
#to make it more reprodusible so you don't have to know or include the observation range
