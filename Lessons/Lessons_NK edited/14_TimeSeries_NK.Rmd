...
PeterPaul.nutrients.surface <- 
  ...
  filter(depth == 0) %>%
  filter(!is.na(tp_ug)) #time series analysis will often run into error if there are NA s in the dataset. 
...
#Interpretation of output: The Ho hypothesis is that there is no monotonic trend. In this case, the p value is low  so we reject the Ho hypothesis. Z is the direction and magnitude of the trend. Because the Z score is positive we know the trend is increasing over time. 
```
...
pettitt.test(Peter.nutrients.surface$tp_ug) # to tell us if there is a shift in the tendancy of the tren d and where that break point happened. This will only give you one change point if there is one. 
# Interpertation of output: Small p value means the test has detected a change point (Ho hypothesis is there is no change point). It has detected a chnage point at 35. Therefore 1 - 34 is a group and 1-35 is a group

# Run separate Mann-Kendall for each change point
mk.test(Peter.nutrients.surface$tp_ug[1:34])
# output: the test does not detects a monotonic trend in this group of data. Ho is accepted because p value is high. The trend is positive over time.
mk.test(Peter.nutrients.surface$tp_ug[35:132])
#output: the test also does not detect a significant monotic trend over time. The trend is negative. 

# Is there a second change point? - in the second group of data
pettitt.test(Peter.nutrients.surface$tp_ug[35:132])
#output: we do detect another change point at 79. This is point 79+35 = 114 of the whole dataset

# Run another Mann-Kendall for the second change point
mk.test(Peter.nutrients.surface$tp_ug[35:113])
#output: This test detects a monotonic trend. The Ho is rejected because the p-value is small. The z score indicates that the trend is a positive trend. 
mk.test(Peter.nutrients.surface$tp_ug[114:132])
#output: the test does not detect a minitinic trend. The Ho is accepted because the p-value is abobe 0.05.

# Run the same test for Paul Lake. 
mk.test(Paul.nutrients.surface$tp_ug)
#output: no significant trend. High p value. 
pettitt.test(Paul.nutrients.surface$tp_ug)
#output: high p value shows there is ni significant change point. 

# Add vertical lines to the original graph to represent change points
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  scale_color_manual(values = c("#7fcdbb", "#253494")) + 
  geom_vline(xintercept = as.Date("1993-05-21"), color = "#253494", lty = 2) +
...
> ANSWER: Mean interprolation - if there is not a lot of variation
Piecewise constant interpolation - for data that is distinct 
Linear interpolation - for data that is continuous
...
USGS.smktest
#output interoretation: p value is small therefore there is a monotonic trend. The z score incdicates we have an increasing trend over time
summary(USGS.smktest)
#output interpretation: It gives a summary of the seasonal trend. The winter months have a significant increasing trend in flow rate, The spring months have no significant trends over time.
```
