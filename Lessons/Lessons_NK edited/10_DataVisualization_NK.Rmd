...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() # very basic theme
print(O3plot4)
```
...
theme_set(mytheme) # you can set the theme and that will be the default for other plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 0, ymax = 50, fill = "green") + #background fill for good
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") +  # background fill for moderate
  geom_point() + #ggplots build layers on top of each other so ordering geom_rect first is useful
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + # the coordinates for the tag good
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + #the coordinates for the tag moderate
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), #specifys the data range to be jan 2017 to dec 2018 
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + # y axis label
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #text angle just for the x axis. hjust means horizontal justification (hjust = 1 moves points to the left)
print(O3plot7)  
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, fill = depth, shape = lakename)) +
  geom_point(alpha = 0.8, size = 3) +
  xlab("Total phosphorous (\U003BCg/L)") +
  ylab("total nitrogen (\U003BCg/L)") +
  #legend(title = "Depth (m)") #figure out how to change the legend title
  scale_shape_manual(values = c(21, 23)) + #specifying the shapes
...
Nutrientplot <-
  ...
  geom_boxplot() +
  scale_x_discrete(labels=c("Paul","Peter"))
