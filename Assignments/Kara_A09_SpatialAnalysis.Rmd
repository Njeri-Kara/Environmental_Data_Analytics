...
> ANSWER: The EPSG code of the dataset is 4269. This is a geographic coordinate system that uses angular coordinate units. The CRS is associated to the datum NAD83.
...
gage_loc.data.raw <- read.csv("./Data/Raw/NWIS_SiteInfo_NE_RAW.csv")

#Reveal the names of the columns
colnames(gage_loc.data.raw)
```
> QUESTION: What columns in the dataset contain the x and y coordinate values, respectively? 

> ANSWER: X values are in dec_long_va and y values are in dec_lat_va
...
gage_loc.data.raw_sf <- st_as_sf(gage_loc.data.raw,coords = c('dec_long_va','dec_lat_va'),crs=4269)

#Reveal the structure
str(gage_loc.data.raw_sf)
```

> QUESTION: What new field(s) appear in the sf dataframe created? What field(s), if any, disappeared?

> ANSWER: The new field, geometry appears. The dec_long_va and dec_lat_va fields disappear. 
...
ggplot() + 
  ...
  geom_sf(data = gage_loc.data.raw_sf,col='blue')
```
...
```{r}
#Read in the data
gage_height.data.raw <-read.csv("./Data/Raw/NWIS_SiteFlowData_NE_RAW.csv")

#Show the column names
colnames(gage_height.data.raw)

#Join location data to it
gage_height_loc.data <- gage_height.data.raw %>% 
  left_join(y = gage_loc.data.raw,by = c("site_no","station_nm")) %>%
  na.omit()

#Convert back to sf dataframe
gage_height_loc.data_sf <- st_as_sf(gage_height_loc.data,coords = c('dec_long_va','dec_lat_va'),crs=4269)
...
ggplot() + 
  ...
  geom_sf(data = gage_height_loc.data_sf,aes(size=gage_ht,col=gage_ht)) +
...
gage_height_loc.data_sf_utm <- st_transform(gage_height_loc.data_sf, c=32614)

#Plot the data
ggplot() + 
  geom_sf(data = counties_sf_utm,col='green') + 
  geom_sf(data = gage_height_loc.data_sf_utm,aes(col=gage_ht)) +
  scale_color_gradient(low = "red", high = "orange")
```

> QUESTION: The shape of Nebraska should look a bit different than the one created in Step 5? Why?

> ANSWER: It should look different because the Nebraska data has been transformed to a projected coordinate system,UTM Zone 14. The data in step 5 is in a geographic coordinate system 
...
```{r}
#Selectiterng Lancaster county
Lancaster.site <- counties_sf_utm %>% 
  filter(NAME %in% c("Lancaster")) 

#Select gages within that county
#Creating a boolean mask 
resultMask <- st_intersects(gage_height_loc.data_sf_utm,
                            Lancaster.site,
                            sparse = FALSE)

lanchaster.gage.data<- gage_height_loc.data_sf_utm[resultMask,]

  
#Plotting with leaflet
#transforming datasets to leaflets native WGS84
Lancaster_wgs84 <- st_transform(Lancaster.site, c=4326)
counties_WGS84 <- st_transform(counties_sf_utm, c=4326)
lanchaster.data_WGS84 <- st_transform(lanchaster.gage.data,c=4326)

leaflet() %>% addTiles() %>% 
  addMarkers(data=lanchaster.data_WGS84,popup = ~as.character(`station_nm`)) %>%
  addPolygons(data=Lancaster_wgs84,weight=1,color='blue') %>% 
...
```{r Spatial Join}
#Join features
county_gage_join <- counties_sf_utm %>% 
  st_join(gage_height_loc.data_sf_utm)

#Show column names
colnames(county_gage_join)

#transform data set to leaflets native WGS84
county_gage_wgs84 <- st_transform(county_gage_join, c=4326)
gage_height_loc_wgs84 <- st_transform(gage_height_loc.data_sf_utm, c=4326)

#Plot
col.length <- length(unique(county_gage_wgs84$NAME))
expand.Palette <- colorRampPalette(brewer.pal(11, "Spectral"))
col.locations <- colorFactor(expand.Palette(col.length),county_gage_wgs84$NAME)

leaflet() %>% 
  addTiles() %>%  
  addPolygons(data = county_gage_wgs84, weight = 2, smoothFactor = 0.5,   
              opacity = 1.0, fillOpacity = 0.5, color = 'white',fillColor = ~col.locations(NAME)) %>% 
  addCircleMarkers(data= gage_height_loc_wgs84,
                   color = 'black',
                   radius=(~gage_ht*0.5),
                   stroke = FALSE, 
                   fillOpacity = 0.5,
                   popup = ~as.character(`gage_ht`))
```
...
```{r}
#Group and summarize
gage_county_summ <- county_gage_join %>%
  group_by(NAME) %>%
  summarize(gage.height.mean = mean((gage_ht),na.rm = TRUE))
  
#Convert result to a simple dataframe
gage_county_sum.df <- st_drop_geometry(gage_county_summ)

#Join summary to County fc
county_gage_summ.join <- left_join(counties_sf,gage_county_sum.df, by='NAME')

#Plot
mapview(county_gage_summ.join['gage.height.mean'])
```
