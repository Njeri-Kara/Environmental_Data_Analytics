...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
  ...
  + Do you have a particular interest in the studied factor level? - fixed effect

  + Have you included all possible levels in the study? - fixed effect
  
  + Do you have interest in the variance among levels? - random effects
...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     ...
                     random = ~1|Week) #This is the random effect of week as a random variable
TempTest.auto
# The random effects output tells us we have a 1.588 std variation between temperatures in one week among all of the weeks
ACF(TempTest.auto)
```
...
TempTest.mixed <- lme(data = PeterPaul.summertemp,
                     ...
                     correlation = corAR1(form = ~ sampledate/lakename|Week, value = 0.423), #we are dividing by lakename bacause both lakes are sampled on the same date
                     #define method as restricted maximum likelihood
                     method = "REML")
summary(TempTest.mixed)
#Output: The intercept is the temperature at Paul lake on the first day of sample date
...
#output - they are bith in group a because there is no statistical difference between them.
...
ggplot(PeterPaul.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  ...
  #scale_x_continuous(breaks = seq(min(PeterPaul.summertemp$sampledate), max(PeterPaul.summertemp$sampledate), by =2 )
```
