...
```{r, results = "hide"}
vignette("dplyr") #to learn more about dplyr
```
...
head(NTL.phys.data.surface1) #original row numbers are saved
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) #new row numbers are developed
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # , can be used instead of an & 
...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999);NTL.phys.data.1999
...
NTL.phys.data.tue.1990s <- filter(NTL.phys.data, year4 < 2000 & year4 > 1989 & lakename == "Tuesday Lake ")
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # sort dataset by depth, default is ascending
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #sorting dataset by depth in decsending order
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #selecting lakename, and sampledate through to temperature_C
...
```{r}
#The data frame is always the first line/section of code
...
```{r}
#DO THIS. MAKE SURE FILE IS IN THE PROCESSED FOLDER
