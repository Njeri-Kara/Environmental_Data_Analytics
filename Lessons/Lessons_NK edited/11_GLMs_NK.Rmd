...
# the qqplot justifys running a one sample t test even though the shapiro test does not hold for normality
...
> ANWSER: Ozone AQI values in 2017 - 2018 are significantly lower that 50 (one amPLE t-test; t= -41.9, df = 184, p< 0.0001)  
...
O3.twosample
O3.twosample$p.value # calling up the p-value of the test output

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name)
summary(O3.twosample2)
#Results
#intercept term ins blackstone, the Bryson is -3.2999 of the blackstone intercept
```
...
O3.onesample.wilcox 
# useful when you don't havea normally distributed population.
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) #result of test is the variances are homogeneous, this fulfills an assuption on equal variance
...
summary(PM2.5.anova2) # results tell us site name is a significant predictor of PM 2.5 variable 

# Run a post-hoc test for pairwise differences
TukeyHSD(PM2.5.anova2) # this test is run on the aov object
...
#to differenciate pairwise diffrerences we can put a label or asterix above Blacktone and another label above Bryson and Triple oak
```
What information does the output give us? How might we report this information in a report?

> ANSWER:(ANOVA; F = 16.16; df = 1898; P<0.0001) The PM.25 values were on average higher in Blackstone than in Bryson and Triple oak. Triple oak and Bryson did not have significantly different values
...
```{r}
TNanova.main <- lm(PeterPaul.nutrients$tn_ug ~ PeterPaul.nutrients$lakename + PeterPaul.nutrients$depth_id) #main effetcs anova is specified by a + between the variables
...
summary(TNanova.main2) # lake and depth id are significant predictors of tn-ug nutrients

TukeyHSD(TNanova.main2)
plot(TukeyHSD(TNanova.main2))
