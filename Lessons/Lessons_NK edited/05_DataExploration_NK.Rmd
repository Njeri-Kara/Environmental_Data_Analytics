...
```{r}
# Load package
library(tidyverse) #Advantage of using Library over require is require will not let you know if there is a problem with the package. Library will.
```
...
str(USGS.flow.data) #viewing the structure of the dataframe
dim(USGS.flow.data) #dimmensions of the dataframe, rows and columns
length(USGS.flow.data)

head(USGS.flow.data) #default rows is 6
head(USGS.flow.data, 10) #to view the first 10 rows
tail(USGS.flow.data, 5) #to view the last 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #subseting a portion of the dataframe to view

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

#stats for certain variables or the entire dataframe
