...
```{r}
# Vectors ---- 
#a comment followed by 4 dashes turns collums into a section
...
vector1[3] # calling the third element in the vector 1 - matrix subsetting

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # take numbers 1 to 20, make 5 rows and 4 collumns with the numbers
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # default is byrow = FALSE ie by collums
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), #naming rows
                                  c("un", "deux", "trois", "cat"))) #naming collumns
matrix3

matrix1[4, ] # subsetting 4th row and all collumns
matrix1[ , 3] # subsetting 3rd collumn and all rows
matrix1[c(12, 14)] #asking the matrix to pull up elements 12 and 14
matrix1[c(12:14)] #asking the matrix to pull up elements 12, 13 and 14
matrix1[2:4, 1:3] # subsetting row 2,3,4 and collums 1,2,3
...
list1[[2]] # calling up the second component of list

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector? - integers
e <- c("red", "white", "red", NA) # What type of vector? - charcter
...
dataframe1[1:2] # collumns 1 and 2 (different from matrix)
dataframe1[1:2,]# rows 1 and 2 
...
> ANSWER: Data section you can view data/elements
values section is used to hold variables/elements

QUESTION: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> ANSWER: it selects the first collumn/variable in the dataframe

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER:It creates a new sectiom
...
* Update your packages regularly! #under packages tab - update
